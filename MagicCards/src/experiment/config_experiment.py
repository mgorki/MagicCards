...
edfDataFolder = (os.getcwd() + "\\data_recorded\\behaviouralData\\edfData")
...
block_max = 2 #10  # Number of blocks to be performed (including one practice block at the beginning) LOW NUMBER ONLY FOR DEBUGING! ADJUST IT IN THE FINAL EXPERIMENT!!!
...
PERCENTAGECORRECTCARD = [10, 10, 15, 15, 20, 20, 25, 25, 30, 30, 35, 35, 40, 40, 45, 45, 50, 50, 55, 55, 60, 60] # The chance (in percent) that the image of the correct card is presented by the computer at the end of a given block. This list must have as many elements as there are blocks.
...
TIME_TOOLATE = 1.5  # After this time (seconds) from target onset response is too late
DURATION_SPAN_FIXATION = (1.0, 1.5)  # Duration range (seconds) of fixation cross. Within this range the duration is random
...
DELAY_INITIAL = 0.5  # Initial delay / ITI (blackscreen) before fixation cross appears (in seconds)
